...
La rectificación homográfica y la detección del sistema de coordenadas de trabajo (WCS, por \textit{Work Coordinate System}) abren el procesamiento. La visión clásica aporta esas referencias geométricas y propone cajas aproximadas. La máscara enviada a la extracción de contornos y a la exportación DXF la genera SAM 2 Hiera Tiny. La comparación incorpora un Random Forest por píxel, cuyos hiperparámetros se escogieron mediante una búsqueda con validación agrupada y cuyo umbral se decidió sobre láminas completas. Las 48 hojas sintéticas se construyeron con 32 identidades gráficas separadas entre entrenamiento, validación y prueba, lo que impide que una misma ilustración reaparezca en más de una partición. La evaluación externa utilizó 13 capturas rectificadas de una única hoja real.

El comportamiento de los métodos cambió con el dominio. En la prueba sintética con identidades no vistas, Random Forest obtuvo un IoU medio de 0,9879, frente a 0,9231 de SAM 2 y 0,9098 de la línea clásica. Sobre las capturas reales la comparación midió concordancia con una referencia canónica asistida, no exactitud frente a un patrón oro independiente: la línea clásica alcanzó 0,9787, SAM 2 obtuvo 0,9536 y Random Forest 0,2881. Aunque no encabezó la comparación real, SAM 2 conservó ocho instancias en todas las capturas y su máscara alimentó la interfaz y la ejecución por lote hasta generar las siluetas del DXF. Diez capturas con WCS produjeron archivos estructuralmente válidos, mientras que en las tres sin una referencia utilizable la exportación quedó bloqueada. Verificar esta cadena mediante cortes físicos es el paso que queda pendiente.
...
Processing begins with homographic rectification and detection of the work coordinate system (WCS). Classical computer vision provides those geometric references and proposes coarse boxes. The mask sent to contour extraction and DXF export is generated by SAM 2 Hiera Tiny. The comparison also includes a pixel-wise Random Forest whose hyperparameters were chosen through a search with grouped validation, while its decision threshold was selected on complete sheets. To prevent the same illustration from appearing in more than one split, the 48 synthetic sheets were built from 32 graphic identities kept disjoint across training, validation and test. External evaluation used 13 rectified captures of a single physical sheet.

Performance changed across domains. On the synthetic test set with unseen identities, mean IoU was 0.9879 for Random Forest, compared with 0.9231 for SAM 2 and 0.9098 for the classical baseline. On the real captures, the comparison measured agreement with an assisted canonical reference rather than accuracy against an independent gold standard. Classical vision reached 0.9787, SAM 2 obtained 0.9536 and Random Forest 0.2881. Although SAM 2 did not lead that comparison, it preserved eight instances in every capture, and its mask was used by both the interface and the batch pipeline to generate the DXF silhouettes. Ten captures with a valid WCS produced structurally valid files, whereas export was blocked for the three captures without a usable reference. Physical cutting trials are still needed to verify the complete chain.

\textbf{Keywords.} Computer vision, Segmentation, Vectorization, Random Forest, SAM 2.

\mainmatter
\input{capitulos/01_introduccion}
\input{capitulos/02_contexto_estado_arte}
\input{capitulos/03_objetivos}
\input{capitulos/04_requisitos}
\input{capitulos/05_metodologia}
\input{capitulos/06_herramienta_software}
\input{capitulos/07_resultados}
\input{capitulos/08_discusion}
\input{capitulos/09_conclusiones}

\appendix
\renewcommand{\appendixname}{ANEXO}
\input{apendices/apendice_a_repositorio}
